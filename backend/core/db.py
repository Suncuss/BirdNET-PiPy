...
class DatabaseManager:
    ...
    def get_latest_detections(self, limit=15):
        ...
        SELECT 
            ...
            week
        FROM detections
        WHERE (id, confidence) IN (
            SELECT id, MAX(confidence) 
            FROM detections 
            GROUP BY group_timestamp, common_name
        )
    ...
    def get_species_sightings(self, limit=10, most_frequent=True):
        ...
        ORDER BY sc.count {order}, d.timestamp DESC
        """
        with self.get_db_connection() as conn:
            cur = conn.cursor()
            cur.execute(query, (limit,))
    ...
    def get_random_detections(self, limit=10):
        query = """
        SELECT d.*
        FROM detections d
        WHERE d.id IN (
