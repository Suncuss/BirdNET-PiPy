...
class TestWeatherService:
    ...
    def test_timezone_extracted_and_cached(self):
        """Test that timezone is extracted from API response and cached."""
        import core.timezone_service as ts
        ts.clear_cache()

        with patch('core.weather_service.requests.get') as mock_get, \
             patch('core.weather_service.os.path.exists', return_value=False), \
             patch('core.weather_service.open', create=True), \
             patch('core.weather_service.os.replace'):

            mock_get.return_value = _create_mock_response(timezone='Europe/London')

            from core.weather_service import WeatherService
            service = WeatherService(51.5, -0.12)

            time.sleep(0.1)

            # Verify timezone was cached
            cached_tz = ts._get_cached_timezone()
            assert cached_tz == 'Europe/London'
            service.stop()

        ts.clear_cache()

    def test_timezone_persisted_to_settings(self):
        """Test that timezone is persisted to settings file."""
        import core.timezone_service as ts
        import json
        ts.clear_cache()

        with patch('core.weather_service.requests.get') as mock_get, \
             patch('core.weather_service.os.path.exists', return_value=True), \
             patch('core.weather_service.os.replace') as mock_replace, \
             patch('builtins.open', create=True) as mock_open:

            # Mock reading existing settings
            mock_open.return_value.__enter__.return_value.read.return_value = json.dumps({
                'location': {'latitude': 51.5, 'longitude': -0.12}
            })

            mock_get.return_value = _create_mock_response(timezone='Europe/Paris')

            from core.weather_service import WeatherService
            service = WeatherService(51.5, 2.35)
    ...
    def test_timezone_not_written_if_unchanged(self):
        """Test that timezone is not written to disk if unchanged."""
        import core.timezone_service as ts
        ts.clear_cache()
        ts.update_cached_timezone('America/New_York')  # Pre-set the cache

        with patch('core.weather_service.requests.get') as mock_get, \
             patch('core.weather_service.os.path.exists', return_value=True), \
             patch('core.weather_service.os.replace') as mock_replace:

            mock_get.return_value = _create_mock_response(timezone='America/New_York')

            from core.weather_service import WeatherService
            service = WeatherService(40.7, -74.0)

            time.sleep(0.1)

            # Verify atomic write was NOT called (timezone unchanged)
