...
class TestDatabaseQueryMethods:
    ...
    def test_get_random_detections(self, test_db_manager):
        """Test get_random_detections() method."""
        # Insert detections for multiple species
        species_list = ['American Robin', 'Blue Jay', 'Northern Cardinal', 'Hooded Warbler']
        
        for species in species_list:
            for i in range(3):  # 3 detections each
                detection = {
        ...
        random_detections = test_db_manager.get_random_detections(limit=2)
        
        assert len(random_detections) == 2
        # Each should be from a different species (first detection of random species)
        species_in_results = [d['common_name'] for d in random_detections]
        assert len(set(species_in_results)) == 2  # 2 unique species
    ...
    def test_empty_database_queries(self, test_db_manager):
        ...
        assert test_db_manager.get_random_detections(5) == []
