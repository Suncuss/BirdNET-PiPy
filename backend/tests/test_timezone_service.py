...
class TestGetTimezoneStr:
    ...
    def test_reads_from_config(self):
        with patch.object(tz_mod, '_safe_mtime', return_value=1000.0), \
             patch.object(tz_mod, 'get_runtime_setting', return_value='Europe/Berlin'):
            assert get_timezone_str() == 'Europe/Berlin'

    def test_returns_utc_when_timezone_is_none(self):
        with patch.object(tz_mod, '_safe_mtime', return_value=1000.0), \
             patch.object(tz_mod, 'get_runtime_setting', return_value=None):
            assert get_timezone_str() == 'UTC'

    def test_returns_utc_when_timezone_is_empty(self):
        with patch.object(tz_mod, '_safe_mtime', return_value=1000.0), \
             patch.object(tz_mod, 'get_runtime_setting', return_value=''):
            assert get_timezone_str() == 'UTC'
...
class TestGetTimezone:
    ...
    def test_returns_zoneinfo(self):
        with patch.object(tz_mod, '_safe_mtime', return_value=1000.0), \
             patch.object(tz_mod, 'get_runtime_setting', return_value='America/New_York'):
            assert get_timezone() == ZoneInfo('America/New_York')

    def test_returns_utc_for_invalid_timezone(self):
        with patch.object(tz_mod, '_safe_mtime', return_value=1000.0), \
             patch.object(tz_mod, 'get_runtime_setting', return_value='Invalid/Timezone'):
...
class TestLocalNow:
    ...
    def test_returns_naive_datetime(self):
        with patch.object(tz_mod, '_safe_mtime', return_value=1000.0), \
             patch.object(tz_mod, 'get_runtime_setting', return_value='UTC'):
            result = local_now()
    ...
    def test_respects_configured_timezone(self):
        with patch.object(tz_mod, '_safe_mtime', return_value=1000.0), \
             patch.object(tz_mod, 'get_runtime_setting', return_value='Pacific/Auckland'):
            nz_now = local_now()
...
class TestMtimeCache:
    ...
    def test_does_not_reload_when_mtime_unchanged(self):
        """get_runtime_setting should only be called once for same mtime."""
        with patch.object(tz_mod, '_safe_mtime', return_value=1000.0), \
             patch.object(tz_mod, 'get_runtime_setting', return_value='America/New_York') as mock_get:
            get_timezone_str()
            get_timezone_str()
    ...
    def test_reloads_when_mtime_changes(self):
        ...
        mock_settings = MagicMock(return_value='America/New_York')

        with patch.object(tz_mod, '_safe_mtime', mock_mtime), \
             patch.object(tz_mod, 'get_runtime_setting', mock_settings):
            assert get_timezone_str() == 'America/New_York'

            mock_mtime.return_value = 2000.0
            mock_settings.return_value = 'Europe/London'
