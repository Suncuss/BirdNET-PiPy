<template>
    <div class="bird-detection-list">
      <h2 class="text-lg font-semibold mb-2">Recent Detections</h2>
      <transition-group name="list" tag="ul" class="space-y-2">
        <li v-for="detection in detections" :key="detection.common_name" 
          ...
          <div class="flex items-center justify-between">
            <div class="flex-grow">
              <div class="font-semibold text-gray-800">{{ detection.common_name }}</div>
              <div class="text-sm text-gray-600 italic">{{ detection.scientific_name }}</div>
            </div>
            <div class="text-right">
              <div class="text-lg font-bold text-green-600">{{ (detection.confidence * 100).toFixed(0) }}%</div>
