...
- Replaced the Werkzeug development server with gunicorn running a single gevent WebSocket worker for the API container, removing the `allow_unsafe_werkzeug` "not for production" override and giving proper connection handling and graceful restarts. The single worker is intentional and load-bearing: live detections fan out via an in-process `socketio.emit` (there is no Redis message queue), so multiple workers would silently drop Live Feed broadcasts to clients connected to a different worker. To keep that one worker responsive under gevent, the rare heavy maintenance jobs — database migration import, audio import, spectrogram backfill, the large uploaded-DB copy, and the duplicate-key preload — now cooperatively yield at batch/file/chunk boundaries; the read-only discovery scans remain a documented accepted residual. Added an unauthenticated `/api/health` liveness route with a docker-compose healthcheck so the recorder and frontend containers wait for the API to be genuinely ready (not merely started), and raised the nginx `/socket.io/` proxy read/send timeout to 3600s so idle Live Feed WebSockets are no longer reaped at nginx's 60s default. Local `python -m core.api` still runs the threading dev server, so the development workflow is unchanged
