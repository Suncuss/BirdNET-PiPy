...
- Added a "customize image" modal on bird detail pages that surfaces multiple Wikimedia thumbnail candidates and lets users pick a different picture for a species; the choice is persisted as a per-species sidecar so it survives across redeploys, and the bird gallery is live-patched on apply without a page reload. The modal grid uses smaller Wikimedia thumbnails (~200 px) to keep candidate loading snappy, and Wikimedia rate-limits (HTTP 429) surface as a friendly retry-later message instead of a raw error
- Fixed the dashboard's Latest Observation spectrogram axis starting above 0 Hz — the lowest visible frequency now consistently starts at 0 Hz so the live canvas and the saved spectrogram PNG line up
- Refined the Latest Observation live spectrogram color and intensity mapping to faithfully match the saved PNG — the canvas now uses matplotlib's actual `Greens_r` ramp (interpolated from ColorBrewer 9-class Greens) instead of an HSL approximation, and intensity is normalized to a per-playback rolling peak in dBFS to mirror the PNG's `Sxx / max_power` step. A gamma curve lifts midtones up the ramp without shifting the dark floor or white peak, and light analyser smoothing damps single-frame FFT jitter so the noise floor reads as a soft wash rather than a flickering grain
- Added click-to-pause on the live spectrogram canvas — clicking the moving spectrogram pauses playback (preserving position and rolling-max calibration) and the play button reappears to resume; there is no separate pause button. A suspended AudioContext is also resumed on play, so a backgrounded-then-resumed tab no longer renders silent visuals
