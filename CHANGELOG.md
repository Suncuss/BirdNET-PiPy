...
- Made the Hourly Activity heatmap a drill-down into the Table view: clicking an hour label opens the Table filtered to that day and hour, and clicking a cell additionally filters by that species (e.g. the 2 PM American Robin cell → just those detections). The Table also gained an "Hour" dropdown (desktop) and a new `hour` (0–23) query parameter on `/api/detections`. The hour labels are an HTML overlay over the Chart.js canvas (canvas text can't be hyperlinked), font-matched to the chart's ticks. Clearing filters now also strips the `date`/`hour`/`species` deep-link params from the URL, so a shared or refreshed "cleared" view stays cleared instead of re-seeding the old filter on the next mount
...
- Replaced the Werkzeug development server with gunicorn + a single gevent WebSocket worker for the API container, removing the `allow_unsafe_werkzeug` "not for production" override and giving proper connection handling and graceful restarts. The single worker is intentional and load-bearing: live detections fan out via an in-process `socketio.emit` (there is no Redis message queue), so multiple workers would silently drop Live Feed broadcasts to clients on a different worker. To keep that worker responsive under gevent, the rare heavy maintenance jobs (database/audio import, spectrogram backfill, large uploaded-DB copy, duplicate-key preload) now cooperatively yield at batch boundaries. Added an unauthenticated `/api/health` liveness route with a docker-compose healthcheck so the recorder and frontend containers wait for the API to be genuinely ready, and raised the nginx `/socket.io/` proxy timeout to 3600s so idle Live Feed WebSockets are no longer reaped at nginx's 60s default. Local `python -m core.api` still runs the threading dev server, so the development workflow is unchanged
