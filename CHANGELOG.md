...
- Fixed Bird Gallery cards intermittently falling back to the default placeholder after the first ~8-9 species — the 4-wide image worker pool added in 0.7.1 fanned out ~8 concurrent Wikimedia API hits (2 per card) per tab open from the Pi's single IP, tripping the unauthenticated burst throttle. Image loads are now serialized; the worker-pool plumbing stays so concurrency can be raised once an upstream cache lands on `/api/wikimedia_image`
- Fixed the Dashboard's Observation Summary card growing taller than the adjacent Latest Observation card at narrow desktop widths — long bird names in the Most Common and Rarest rows wrapped to a second line. Each row is now a single-line flex layout with the value truncating on overflow; the full common name is still reachable on hover and clicking the truncated row still opens the species page
- Fixed the Distribution chart on bird detail pages occasionally rendering blank when the browser window was resized quickly — two competing resize handlers were fighting Chart.js's own ResizeObserver: an `onResize` option doing a redundant `chart.update('none')` on every internal resize tick, and a separate debounced window listener that on >100 px width change destroyed and recreated the Chart via an async fetch, racing with itself and occasionally latching a transient 0-width canvas. Both were removed in favor of Chart.js's `responsive: true` + built-in ResizeObserver, with a narrow follow-up that keeps `maxTicksLimit` in sync when the window crosses the mobile-portrait breakpoint by patching the option in place and calling `chart.update('none')` — no destroy/recreate, no refetch
