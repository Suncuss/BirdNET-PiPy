...
- Fixed Settings showing "Waiting for services to report the active model" and, after returning to the tab or reopening the page, a "Could not refresh settings" or "Failed to load version information" error while an update or restart was in progress. The expected outage is now reported only by the progress banner, and the page catches up by itself when an update ends without a reload.
